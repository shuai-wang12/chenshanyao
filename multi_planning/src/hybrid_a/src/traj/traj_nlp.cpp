...
void TrajectoryNLP::BuildNLP() {
  ...
  auto infeasibility = sumsqr(SX::vertcat(
      {g_x_kin, g_y_kin, g_theta_kin, g_v_kin, g_phi_kin, g_a_kin}));

  // w_u 控制项权重
  SX f_obj =
      //   sumsqr(x - p_ref_x) + sumsqr(y - p_ref_y) + config_.opti_w_r_theta *
      //   sumsqr(theta - p_ref_theta) +
      config_.opti_w_u * (sumsqr(jerk) + config_.opti_w_rw * sumsqr(omega)) +
      p_inf_w * infeasibility;

  SX p = SX::vertcat({p_inf_w});
  SX opti_x = SX::vertcat({x, y, theta, v, phi, a, omega, jerk});
...
double TrajectoryNLP::solve(double inf_w, const Constraints& constraints,
  ...
  for (int i = 1; i < config_.nfe - 1; i++) {
    /* 前后两点的速度都小于0，那么将当前点的速度上限置为0
      认为车辆此时在倒车
      但是如果前一点速度大于0，后一点速度小于0，表示车辆在前进和倒车状态中发生了转换
      此时将速度下限置为0是否合理？ */
  ...
  //变量边界
  arg["lbx"] = DM::vertcat(
      {lb_x, lb_y, lb_theta, lb_v, lb_phi, lb_a, lb_omega, lb_jerk});
  arg["ubx"] = DM::vertcat(
      {ub_x, ub_y, ub_theta, ub_v, ub_phi, ub_a, ub_omega, ub_jerk});
  //初值
  arg["x0"] = DM::vertcat({guess.x, guess.y, guess.theta, guess.v, guess.phi,
                           guess.a, guess.omega, guess.jerk});
  //可行性约束权重
  //   DM ref_x(config_.nfe, 1), ref_y(config_.nfe, 1), ref_theta(config_.nfe,
  //   1); for (int i = 0; i < config_.nfe; i++) {
  //     ref_x(i) = reference.data()[i].x;
  //     ref_y(i) = reference.data()[i].y;
  //     ref_theta(i) = reference.data()[i].theta;
  //   }
  arg["p"] = DM::vertcat({inf_w});
  res = solver_(arg);
  DM opt = res.at("x");
